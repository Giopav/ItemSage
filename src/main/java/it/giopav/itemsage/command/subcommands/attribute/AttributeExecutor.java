...
public class AttributeExecutor extends AbstractAttribute implements ExecutorInterface {
    @Override
    ...
    private boolean sendAllAttributes(Player player, ItemStack mainHandItem) {
    ...
    private boolean sendSlotAttributes(Player player, String[] args, ItemStack mainHandItem) {
        args[1] = args[1].toUpperCase();
        if (!mainHandItem.getItemMeta().hasAttributeModifiers()) {
            player.sendMessage(ChatColor.RED + "This item doesn't contain attributes.");
            return false;
        }
        EquipmentSlot equipmentSlot = AbstractAttribute.getEquipmentSlotValue(args[1]);
    ...
    private boolean sendAttribute(Player player, String[] args, ItemStack mainHandItem) {
        ...
        Attribute attribute = AbstractAttribute.getAttributeValue(args[2]);
        if (attribute == null) {
    ...
    private boolean redirect(Player player, String[] args, ItemStack mainHandItem) {
        if (args[2].equalsIgnoreCase("add")) {
    ...
    private boolean redirectAdd(Player player, String[] args, ItemStack mainHandItem) {
    ...
    private boolean addAttribute(Player player, ItemStack mainHandItem, String equipmentSlotString,
                                        String attributeString, String amountString, String operationString) {
        equipmentSlotString = equipmentSlotString.toUpperCase();
        EquipmentSlot slot = AbstractAttribute.getEquipmentSlotValue(equipmentSlotString);
        if (slot == null) {
        ...
        Attribute attribute = AbstractAttribute.getAttributeValue(attributeString);
        if (attribute == null) {
        ...
        AttributeModifier.Operation operation = AbstractAttribute.getOperationValue(operationString);
        if (operation == null) {
    ...
    private boolean editAttribute(Player player, ItemStack mainHandItem, String[] args) {
        EquipmentSlot slot = AbstractAttribute.getEquipmentSlotValue(args[1]);
        if (slot == null) {
    ...
    private Component attributeMessage(Attribute attribute, AttributeModifier attributeModifier) {
